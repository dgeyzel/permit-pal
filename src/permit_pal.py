...
INPUT_PROMPT = []
INPUT_PROMPT.append('I want to open a restaurant in Atlanta, Georgia.')
INPUT_PROMPT.append('I want to open a restaurant in San Diego, CA.')
INPUT_PROMPT.append('I want to remodel a historic home in Atlanta, GA.')
INPUT_PROMPT.append('I want to become a barber in New York City, NY')
INPUT_PROMPT.append('I want to become a barber in Topeka, Kansas')

# List of Gemini models that could be used
GEMINI_MODEL = []
GEMINI_MODEL.append("gemini-2.5-pro")
GEMINI_MODEL.append("gemini-2.5-flash")
GEMINI_MODEL.append("gemini-2.5-flash-lite")
GEMINI_MODEL.append("gemini-3-pro-preview")
GEMINI_MODEL.append("gemini-3-flash-preview")
...
async def main():
    ACTION_PROMPT = INPUT_PROMPT[2]
    ACTIVE_MODEL = GEMINI_MODEL[4]
    additional_context = " "
    if RAG_ENABLED:
        additional_context = await rag_utils.get_context(ACTION_PROMPT)
    ...
    print(f"Starting main {ACTIVE_MODEL} model execution.")
    start = time.perf_counter()
    ai_msg = gemini_ai_model.invoke(messages)
    end = time.perf_counter()
    print(f"Main {ACTIVE_MODEL} model execution time : \
